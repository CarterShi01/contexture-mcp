...
- Add a **PreProcess** or **PostProcess** when a Role needs separately disclosed
    preparation or finishing equipment. They specialize Role and belong in
    `pre_process=...` / `post_process=...`, not among alternative work branches.
...
author their constructors like any Role: give each business instructions and
compose dedicated Skills/Tools or reference shared Tools with `uses`.
...
from contexture import PostProcess, Role, Tool
...
class TaskPublication(PostProcess):
    def __init__(self) -> None:
        super().__init__(
...
class _DefaultPostProcess(Enum):
    TASK = "task"
...
class PublishingTaskWorker(TaskWorker):
    def __init__(
        self,
        post_process: PostProcess | None | _DefaultPostProcess = _DefaultPostProcess.TASK,
    ) -> None:
        if isinstance(post_process, _DefaultPostProcess):
            post_process = TaskPublication()
        super().__init__(post_process=post_process)
```

`TaskWorker()` has no PostProcess. `PublishingTaskWorker()` constructs a fresh
default; `PublishingTaskWorker(post_process=None)` explicitly disables it.
...
`post_process` member. Share external services through Channels or `uses`.
...
Opening the owner shows a normal routing card in `roles` and a `post_process`
string naming its real ref. The framework appends a fixed, recognizable block
requiring `contexture_open` before finishing. `Role.instructions` remains your
business text; the PostProcess procedure and capabilities stay hidden until
opened. Opening does not execute the procedure or prove success; only Tools
execute, with existing approval and authorization requirements.

Without a process member, ROUTE/ACTIVE output and obligations are unchanged. There
is no framework enabled flag, no-op, automatic finish hook, or guarantee that an
external Agent follows the contract. A parent's process member does not
implicitly apply to contained children. Explicit nesting is ordinary acyclic
containment; opening one directly adds no extra contract unless it declares its
own process member.

Selecting an owner includes its process subtrees. Selecting one alone exposes
neither the owner nor the owner's obligation. Prompt-only owners and their
process members remain hidden from ordinary model disclosure; framework composition refuses an unavailable process card
rather than leaking its ref.

PreProcess must hold a procedure the Agent should perform, never an invariant
the application must guarantee. Enforce guarantees in the Tool that would
otherwise violate them. After preparation, the framework instruction explicitly
sends the Agent back to the owner's instructions. See
[ADR 023](adr/023-process-members-and-instruction-emphasis.md).

Applications can mark a hard rule under their own authority without claiming
it came from the framework:

```python
from contexture import binding_instruction

instructions = binding_instruction(
    "operations policy",
    "Never claim a deployment succeeded without its observed receipt.",
)
```

Use this only where code cannot enforce the rule. The source must identify the
application authority and cannot begin with `contexture`.
...
These lower-case *publications* are exposure pointers, not process Roles.
